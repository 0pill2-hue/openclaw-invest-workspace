...
CONFIG_PATH = BASE / "invest/config/stage05_auto_capture_v3_16_kr.yaml"
PREV_JSON = VALIDATED / "stage05_baselines_v3_14_kr.json"
OUT_JSON = VALIDATED / "stage05_baselines_v3_16_kr.json"

LOG_DIR = BASE / "reports/stage_updates/logs"
SCAN_LOG = LOG_DIR / "stage05_no_whitelist_scan_v3_16_kr.log"
UNIVERSE_LOG = LOG_DIR / "stage05_dynamic_universe_v3_16_kr.json"
...
def _series_metrics_from_returns(monthly_returns: pd.Series) -> dict[str, float]:
    ...
    if monthly_returns.empty:
        return {"total_return": 0.0, "mdd": 0.0, "asset_multiple": 1.0}
    ...
    for v in eq.values:
        peak = max(peak, float(v))
        mdd = min(mdd, float(v) / max(1e-12, peak) - 1.0)
    return {"total_return": total_ret, "mdd": float(mdd), "asset_multiple": float(eq.iloc[-1])}
...
def evaluate_high_density_advantage(
    ...
    base_idx = pd.Series({pd.to_datetime(k): float(v) for k, v in runs["numeric"].monthly_returns.items()}).sort_index().index
    if base_idx.empty:
        detail = {
            "enabled": True,
    ...
    target_mask = oos_mask & high_mask
    ...
    for model in ["numeric", "qualitative", "hybrid"]:
        monthly = pd.Series({pd.to_datetime(k): float(v) for k, v in runs[model].monthly_returns.items()}).sort_index()
        monthly = monthly.reindex(base_idx).fillna(0.0)
        seg = monthly[target_mask]
        met = _series_metrics_from_returns(seg)
        sell_trades = [t for t in runs[model].trades if pd.to_datetime(t.get("sell_date", "1900-01-01")).strftime("%Y-%m") in month_keys]
        met["turnover_proxy"] = float(len(sell_trades) / max(1, len(target_dates)))
        met["samples"] = int(len(target_dates))
        metrics_by_model[model] = met
...
def main() -> int:
    ...
    for r_cfg in rounds:
        ...
        high_density_pass, high_density_detail = evaluate_high_density_advantage(runs, density_map, cfg_main)
    ...
    payload = {
        ...
        "policy_enforcement": {
            ...
            "epsilon": EPSILON,
            "repeat_terminate_condition": "non_numeric_top_valid_and_overfit_guard_pass_and_high_density_advantage_pass",
