...
def _quarantine_ohlcv_rows(code: str, bad_df: pd.DataFrame):
    """
        ...
        Role: 오염되거나 무효한 OHLCV 데이터를 검역소(quarantine) 폴더로 분리 저장한다.
        Input: code (종목코드), bad_df (오염된 데이터프레임)
        Output: None (파일 저장)
        Author: 조비스 (Flash)
        Date: 2026-02-18
...
def fetch_all_ohlcv():
    ...
    legacy_output_dir = 'invest/data/raw/kr/ohlcv'  # backward-compat mirror
