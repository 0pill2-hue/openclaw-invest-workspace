...
CONFIG_PATH = BASE / "invest/config/stage05_auto_capture_v3_19_kr.yaml"
...
class ModelRun:
    ...
    supercycle_trace: list[dict[str, Any]]
...
def compute_factor_values(
    ...
    ret_5 = float(c.pct_change(5).iloc[-1])
    ret_s = float(c.pct_change(lock.ret_short).iloc[-1])
    ret_20 = float(c.pct_change(20).iloc[-1]) if len(c) >= 25 else ret_s
    ret_m = float(c.pct_change(lock.ret_mid).iloc[-1])
    ret_120 = float(c.pct_change(120).iloc[-1]) if len(c) >= 130 else ret_m
    ...
    traded_value = (c * v).astype(float)
    tv_ma60 = float(traded_value.rolling(60).mean().iloc[-1]) if len(traded_value) >= 60 else float(traded_value.mean())
    turnover_ratio = float(traded_value.iloc[-1] / max(1e-9, tv_ma60)) if len(traded_value) > 0 else 1.0
    ...
    up_ratio = float(c.pct_change().shift(lag).tail(20).gt(0.0).mean())
    ...
    # v3_19: turnaround + supercycle features
    prior_weakness = float(np.clip(max(0.0, -ret_120) + max(0.0, -breakout), 0.0, 1.0))
    rebound_strength = float(np.clip((ret_20 - ret_m + 0.05) / 0.30, 0.0, 1.0))
    flow_pos = float(np.clip((flow + 1.0) / 2.0, 0.0, 1.0))
    turnaround_score = float(np.clip(0.45 * prior_weakness + 0.35 * rebound_strength + 0.20 * flow_pos, 0.0, 1.0))

    accel_core = float(np.clip((ret_s - ret_m + 0.03) / 0.20, 0.0, 1.0))
    trend_component = float(np.clip((ma_f / (ma_s + 1e-9) - 0.98) / 0.08, 0.0, 1.0))
    turnover_component = float(np.clip((turnover_ratio - 0.80) / 0.80, 0.0, 1.0))
    supercycle_accel = float(np.clip(0.40 * accel_core + 0.35 * trend_component + 0.25 * turnover_component, 0.0, 1.0))

    up_ratio20 = float(c.pct_change().tail(20).gt(0.0).mean()) if len(c) >= 25 else 0.5
    persistence_base = float(np.clip((ret_m + 0.02) / 0.20, 0.0, 1.0))
    up_component = float(np.clip((up_ratio20 - 0.35) / 0.45, 0.0, 1.0))
    persistence_score = float(np.clip(0.40 * trend + 0.30 * persistence_base + 0.20 * up_component + 0.10 * flow_pos, 0.0, 1.0))

    momentum_decel = float(max(0.0, (ret_m - ret_s)))
    breadth_weak = float(max(0.0, 0.45 - up_ratio20))
    turnover_break = float(max(0.0, 0.90 - turnover_ratio))
    bend_risk = float(
        np.clip(
            0.45 * np.clip(momentum_decel / 0.12, 0.0, 1.0)
            + 0.30 * np.clip(breadth_weak / 0.25, 0.0, 1.0)
            + 0.25 * np.clip(turnover_break / 0.50, 0.0, 1.0),
            0.0,
            1.0,
        )
    )
    ...
    return {
        ...
        "supercycle_accel": supercycle_accel,
        "persistence_score": persistence_score,
        "bend_risk": bend_risk,
        "momentum_decel": momentum_decel,
        "breadth_weak": breadth_weak,
        "turnover_break": turnover_break,
        "turnover_ratio": turnover_ratio,
        "up_ratio20": up_ratio20,
        "trend_signal": float(trend),
        "ret_5": float(0.0 if math.isnan(ret_5) else ret_5),
        "ret_20": float(0.0 if math.isnan(ret_20) else ret_20),
        "ret_mid": float(0.0 if math.isnan(ret_m) else ret_m),
    }
