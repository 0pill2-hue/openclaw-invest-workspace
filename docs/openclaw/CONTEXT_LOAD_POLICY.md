...
- TASKS SSOT: `runtime/tasks/tasks.db` (`python3 scripts/tasks/db.py`, 게이트: `python3 scripts/tasks/gate.py`)
- DIRECTIVES SSOT: `runtime/directives/directives.db` (`python3 scripts/directives/db.py`, 게이트: `python3 scripts/directives/gate.py`)
...
- 보고/운영 이슈: OPERATING_GOVERNANCE + `scripts/directives/db.py list` 기준 확인
- 시스템/구조 변경: OPENCLAW_SYSTEM_BASELINE + WORKSPACE_STRUCTURE
- 뇌 역할/폴백 기준 확인: docs/openclaw/BRAINS.md
