...
| `stage01_data_collection.md` | canonical collection appendix | collector ↔ output path/source map + DB/archive 기준 최소 artifact 스키마 | 원천/산출 포맷이 바뀌면 여기 반영 |
...
- collector별 raw rel_path / DB archive namespace와 **최소 artifact 스키마**는 `stage01_data_collection.md` 우선
- 미해결 TODO는 `TODO.md`에만 남긴다

## Stage1 한 줄 요약
- 역할: 외부 원천을 수집해 `master/db(raw archive + PDF index)/runtime/logs/reports` 기준선을 만든다. (`raw/`는 수집기 호환 미러이며 Telegram PDF는 bucketed flat artifact로 남긴다)
...
- [TODO.md](./TODO.md)
- DB raw archive: `invest/stages/stage1/outputs/db/stage1_raw_archive.sqlite3`
