...
- `docs/operations/context/` : 사람이 읽는 context 관련 카테고리 인덱스 (기본 로드 제외)
- `docs/operations/governance/` : 사람이 읽는 governance 관련 카테고리 인덱스 (기본 로드 제외)
- `docs/operations/runtime/` : 사람이 읽는 runtime/program/guard 관련 카테고리 인덱스 (기본 로드 제외)
- `docs/operations/skills/` : 사람이 읽는 개별 스킬 설명/템플릿 문서
- `docs/operations/orchestration/` : 사람이 읽는 orchestration 설명/다이어그램 문서 (기본 로드 제외)
...
- explainer/diagram 문서는 canonical 규칙을 장문 복제하지 않고, 링크/배경/시각화만 담당한다.
- 역할 없는 중복 인덱스는 삭제한다.
...
- 세부 기준은 `docs/operations/governance/PRIVATE_LOCAL_DOCS_POLICY.md`를 따른다.
