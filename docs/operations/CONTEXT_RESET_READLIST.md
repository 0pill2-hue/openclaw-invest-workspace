...
이 문서는 리셋 직후 무엇을 다시 읽을지 최소 순서를 정의한다.
임계치/정책은 `CONTEXT_POLICY.md`, 로딩 트리거는 `CONTEXT_LOAD_POLICY.md`를 따른다.
