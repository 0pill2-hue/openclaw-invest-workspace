...
`local_brain_guard`는 이 문서가 정의하는 메인브레인가드의 **하위 복구 컴포넌트**다.
...
- 복구: llama 재시작 → 2초 대기 → Gateway 재시작 → 재점검
- 성격: **컴포넌트 복구 가드**
...
- `ok`: 상위 메인브레인가드 최종 판정
- `summary`: `local_brain/telegram/watchdog/auto_dispatch/current_task`의 OK/FAIL 요약
- `failed_components`: 실패 컴포넌트 목록
- `issues`: 컴포넌트별 기계 판독용 이유 목록
- `alert`: heartbeat가 그대로 전달할 수 있는 경고 문구
- `checks.*`: 세부 근거(JSON)
...
- 서브를 붙인 직후 task에는 반드시 phase/deadline을 남긴다.
  - 예: `subagent_running` 또는 `awaiting_callback`
  - callback deadline(`resume_due`)을 함께 기록한다.
- 메인은 이 이벤트를 받으면 같은 턴에서:
  1) child report 확인
  2) proof 반영
  3) task 상태 전이(DONE/REWORK/BLOCKED)
  를 즉시 처리한다.
- deadline을 넘겼는데 callback이 없으면 watchdog은 더 기다리지 않고 `BLOCKED`로 전환한다.
- 기본 텍스트 형식 예시:
