...
## 1) SSOT / Ledger
### `scripts/tasks/db.py`
- 역할: TASKS SQLite 원장 등록/상태전이/요약/render SSOT + retryable/cleared BLOCKED backlog 재큐잉
...
- 관련 문서: `docs/operations/context/CONTEXT_POLICY.md`, `docs/operations/context/CONTEXT_LOAD_POLICY.md`, `docs/operations/context/CONTEXT_HANDOFF_FORMAT.md`

## 3) Dispatch / Orchestration
### `scripts/tasks/dispatch_tick.py`
- 역할: assign-pool → orchestrator 호출 → ticket close 또는 backgrounded(detached waiting) 상태 판정
...
- 주 입력: `task_id`, `source`, `summary`, `phase`, proof path
- 주 출력: `runtime/tasks/<ticket>.md`, taskdb phase/touch/release 갱신
- 실패 시: non-zero 반환, JSON error 출력
- 관련 문서: `TASKS.md`, `scripts/README.md`, `docs/operations/orchestration/NONIDLE_ORCHESTRATION_GUIDE.md`
...
- 역할: **진짜 stale/blocker만** BLOCKED 전환 (`delegated_to_subagent`/`subagent_running`/`awaiting_callback`/`long_running_execution` 같은 nonterminal waiting phase는 age만으로 막지 않고, `resume_due` 초과 시에만 blocker로 승격) + watchdog성/시간창 만료/선행조건 해소된 BLOCKED backlog 자동 requeue
- 주 입력: `runtime/tasks/tasks.db`
- 주 출력: 상태 전이 JSON(`moved`, `requeued`)
...
- 관련 문서: `runtime/tasks/README.md`, `docs/operations/runtime/MAIN_BRAIN_GUARD.md`
...
- 관련 문서: `docs/operations/runtime/MAIN_BRAIN_GUARD.md`, `HEARTBEAT.md`

## 6) 문서 링크 원칙
- `OPERATIONS_BOOK.md`: 이 문서를 총람 링크로 포함
- 개별 운영 문서: 필요한 프로그램만 1~2줄로 언급하고 이 문서 링크
- `scripts/README.md`: 파일 경로와 실행 예시 중심 유지
- `docs/operations/orchestration/`: 사람용 구조 설명/다이어그램만 유지
