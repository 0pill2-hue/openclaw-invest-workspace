...
inputs:
  ...
  validation_script: invest/scripts/validate_refine_independent.py
outputs:
  validate_manifest: invest/reports/data_quality/manifest_stage2_validate_*.json
  validate_log: reports/stage_updates/logs/
run_command:
  - python3 invest/scripts/validate_refine_independent.py

quality_gates:
  - id: QG-03-01 (Independent Verdict)
    check: "verdict_*.json -> status == 'PASS'"
  ...
  - id: QG-03-04 (Sampling Audit)
    ...
    path: "invest/scripts/qc_cleaning_10pct.py output"
...
repro_checklist:
  - script: "invest/scripts/validate_refine_independent.py"
