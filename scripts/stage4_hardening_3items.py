...
VAL_BASE = Path("invest/data/value/stage3")
MASTER_KR = Path("invest/data/master/kr_stock_list.csv")
OUT_MD = Path("reports/qc/STAGE4_HARDENING_3ITEMS_20260218.md")
OUT_JSON = Path("reports/qc/STAGE4_HARDENING_3ITEMS_20260218.json")
...
def main():
    ...
    md = []
    md.append("# STAGE4 HARDENING 3ITEMS (2026-02-18)\n")
    ...
    for line in out["stage4_summary_5lines"]:
        md.append(f"- {line}\n")
    ...
    manifest_path = Path('invest/reports/data_quality') / f'manifest_stage4_hardening_{ts}.json'
    write_run_manifest(
        run_type='stage4_hardening_3items',
