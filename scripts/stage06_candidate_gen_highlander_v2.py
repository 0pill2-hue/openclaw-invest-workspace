...
# v2 tuning targets
TREND_WEIGHT = 0.50  # 30% -> 50%
FLOW_WEIGHT = 0.22
FIN_WEIGHT = 0.10
MOM_WEIGHT = 0.18
LEADER_BONUS = 0.12
BUZZ_PENALTY_MAX = 0.20
...
def load_symbol_panel(code: str) -> pd.DataFrame | None:
    ...
    df["turnover5"] = (df["Close"] * df["Volume"]).rolling(5).mean()
...
def build_sector_map(universe: list[str]) -> dict[str, str]:
    cache = CACHE_DIR / "highlander_sector_map_cache.csv"
    if cache.exists():
        cdf = pd.read_csv(cache, dtype={"code": str, "sector": str})
        d = {r["code"]: r["sector"] for _, r in cdf.iterrows()}
        return {c: d.get(c, "UNKNOWN") for c in universe}
    ...
    for market in ["KOSPI", "KOSDAQ"]:
        ...
        for idx in idx_list:
            try:
                ...
                # broad/style indices 제외, 업종 중심만 사용
                if any(k in name for k in ["코스피", "코스닥", "KRX", "200", "100", "50", "대형", "중형", "소형", "배당", "ESG"]):
                    continue
                members = stock.get_index_portfolio_deposit_file(idx)
                for code in members:
                    if code in universe and code not in sector_map:
                        sector_map[code] = name
            except Exception:
                continue

    out = pd.DataFrame({"code": list(sector_map.keys()), "sector": list(sector_map.values())})
    out.to_csv(cache, index=False, encoding="utf-8-sig")
    return {c: sector_map.get(c, "UNKNOWN") for c in universe}


def load_monthly_market_cap(universe: list[str]) -> pd.DataFrame:
    CACHE_DIR.mkdir(parents=True, exist_ok=True)
    cache = CACHE_DIR / "highlander_monthly_mcap_cache.csv"
    if cache.exists():
    ...
    for d in pd.date_range(START, END, freq="ME"):
        ds = d.strftime("%Y%m%d")
        try:
            mc = stock.get_market_cap_by_ticker(ds, market="ALL")
            if mc is None or mc.empty:
                continue
            x = mc.reset_index().rename(columns={"티커": "code", "시가총액": "mcap"})
            x = x[x["code"].isin(universe)][["code", "mcap"]].copy()
            x["Date"] = pd.Timestamp(d)
            rows.append(x)
        except Exception:
            continue
    ...
    if not rows:
        return pd.DataFrame(columns=["Date", "code", "mcap"])
    out = pd.concat(rows, ignore_index=True)
    out["mcap"] = pd.to_numeric(out["mcap"], errors="coerce").fillna(0.0)
    out.to_csv(cache, index=False, encoding="utf-8-sig")
    return out
...
def build_rebalance_table(universe: list[str]) -> pd.DataFrame:
    ...
    # trend score 강화: 60/120 정배열 + 이격도
    panel["trend_stack"] = ((panel["Close"] > panel["ma60"]) & (panel["ma60"] > panel["ma120"]))
    panel["trend_gap"] = (panel["Close"] / panel["ma120"] - 1).clip(-0.3, 0.8)
    panel["trend_score"] = 0.6 * panel["trend_stack"].astype(float) + 0.4 * panel["trend_gap"].fillna(0.0)

    # buzz proxy + volatility penalty (뉴스데이터 부재로 turnover spike를 buzz proxy 사용)
    panel["buzz_proxy"] = (panel["turnover5"] / panel["turnover20"]).replace([np.inf, -np.inf], np.nan).fillna(1.0)
    panel["buzz_proxy"] = panel["buzz_proxy"].clip(0.2, 5.0)

    # sector map + market cap rank
    sector_map = build_sector_map(universe)
    panel["sector"] = panel["code"].map(sector_map).fillna("UNKNOWN")

    mcap = load_monthly_market_cap(universe)
    if not mcap.empty:
        panel = panel.merge(mcap, on=["Date", "code"], how="left")
    panel["mcap"] = pd.to_numeric(panel.get("mcap", 0.0), errors="coerce").fillna(0.0)

    panel = panel.sort_values(["Date", "code"]).reset_index(drop=True)

    # 월별 섹터 내 시총순위 + 동반상승 leader bonus flag
    panel["sector_rank"] = panel.groupby(["Date", "sector"])["mcap"].rank(ascending=False, method="first")
    panel["is_sector_leader"] = panel["sector_rank"].between(1, 3, inclusive="both")
    panel["sector_up"] = panel["mom60"].fillna(-9) > 0
    g = panel.groupby(["Date", "sector"], as_index=False)["sector_up"].sum().rename(columns={"sector_up": "sector_up_count"})
    panel = panel.merge(g, on=["Date", "sector"], how="left")
    panel["leader_bonus_flag"] = panel["is_sector_leader"] & (panel["sector_up_count"] >= 2)
...
def simulate(panel: pd.DataFrame, params: Params, start: pd.Timestamp, end: pd.Timestamp) -> tuple[pd.Series, pd.DataFrame]:
    ...
    # date-wise normalized pieces
    p["mom_rank"] = p.groupby("Date")[mom_col].rank(pct=True)
    p["flow_rank"] = p.groupby("Date")["flow20"].rank(pct=True)
    p["trend_rank"] = p.groupby("Date")["trend_score"].rank(pct=True)
    p["vol_rank"] = p.groupby("Date")["vol20"].rank(pct=True)
    p["buzz_rank"] = p.groupby("Date")["buzz_proxy"].rank(pct=True)

    p["base_score"] = (
        TREND_WEIGHT * p["trend_rank"].fillna(0)
        + FLOW_WEIGHT * p["flow_rank"].fillna(0)
        + MOM_WEIGHT * p["mom_rank"].fillna(0)
        + FIN_WEIGHT * (p["fin_score"].fillna(0) / 3.0)
    )

    # Buzz 높고 변동성 높으면 감점 (단기 테마 억제)
    buzz_vol = (p["buzz_rank"].fillna(0) * p["vol_rank"].fillna(0))
    p["buzz_vol_penalty"] = (BUZZ_PENALTY_MAX * buzz_vol).clip(0, BUZZ_PENALTY_MAX)

    # Sector leader bonus
    p["leader_bonus"] = np.where(p["leader_bonus_flag"], LEADER_BONUS, 0.0)

    p["score"] = p["base_score"] + p["leader_bonus"] - p["buzz_vol_penalty"]
...
def main() -> int:
    ...
    out_json = OUT_DIR / "stage06_highlander_v2_result.json"
    out_equity = OUT_DIR / "stage06_highlander_v2_equity.csv"
    out_picks = OUT_DIR / "stage06_highlander_v2_picks.csv"

    payload = {
        "stage": 6,
        "model_name": "Highlander_Aggressive_Momentum_WFO_v2",
        "result_grade": "VALIDATED",
        "test_only": False,
        "period": {"start": str(START.date()), "end": str(END.date())},
        "logic_tuning": {
            "trend_weight": TREND_WEIGHT,
            "buzz_vol_penalty_max": BUZZ_PENALTY_MAX,
            "leader_bonus": LEADER_BONUS,
            "note": "buzz uses turnover-spike proxy due to lack of historical ticker-level news frequency",
        },
